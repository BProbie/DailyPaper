...
import com.probie.dailypaper.dailypaper.api.IDailyPaperFunction;
...
public class DailyPaperFunction implements IDailyPaperFunction {
    ...
    public boolean checkRenewDailyPaper() {
        return Renew.getInstance().turnVersionToNumber(RenewConfig.getInstance().getLocalRemoteDB().get("VERSION", dailyPaper.getVERSION())) > Renew.getInstance().turnVersionToNumber(dailyPaper.getVERSION());
    }
