...
import com.probie.dailypaper.dailypaper.api.IDailyPaperData;
...
public class DailyPaperData implements IDailyPaperData {
     ...
     * 参数
     * */
    private Supplier<Object> promptInformationPrompt = () -> ("你是DailyPaper日常壁纸软件应用的AI小助手小Day"
            + "\n" + "你的任何回复都不要使用Markdown语法，输出的代码也不要使用代码块格式"
            + "\n" + "现在是%d年%d月%d日%d时%d分%d秒，星期%d，%s".formatted(ComputerSystem.getInstance().getDate(Date.YEAR), ComputerSystem.getInstance().getDate(Date.MONTH), ComputerSystem.getInstance().getDate(Date.DAY), ComputerSystem.getInstance().getDate(Date.HOUR), ComputerSystem.getInstance().getDate(Date.MINUTE), ComputerSystem.getInstance().getDate(Date.SECONDE), ComputerSystem.getInstance().getDate(Date.SUNDAY), ComputerSystem.getInstance().getHoliDay()));
    private Supplier<Object> promptIfImagePrompt = () -> ("请你根据用户输入的上下文信息帮我推测判断出用户现在是否需要生成图像，是则回答单个字“是”，否则回答单个字“否”，无法推断或模糊不清则一律回答单个字“否”，以下是用户输入的上下文信息：");
    private Supplier<Object> promptSpawnImagePrompt = () -> ("请你根据用户输入的上下文信息整理回复用户想生成图像的描述信息，稍微简短，尽量详备，并只需要回复要生成的图像的描述信息即可，以下是用户输入的上下文信息：");
    private Supplier<Object> promptSpawnImageResultPrompt = () -> ("请你根据用户当前输入的用于生成图像提示词信息，返回该图像生成成功的回复，以下是用户输入的提示词信息：");
    private Supplier<Object> promptDelineateImagePrompt = () -> ("这是什么？直接回答，稍微简短，尽量详备。");
    private Supplier<Object> promptSpawnDailyWallpaperPrompt = () -> ("现在是%d年%d月%d日%d时%d分%d秒，星期%d，%s。\n如果有可能的话，请你最大化地综合现在的时间地点、日期星期、时节时令、节气节日、风俗风气、历史文化、气氛氛围等众多因素，结合用户的喜爱偏好：%s。\n生成一张电脑桌面高清壁纸。".formatted(ComputerSystem.getInstance().getDate(Date.YEAR), ComputerSystem.getInstance().getDate(Date.MONTH), ComputerSystem.getInstance().getDate(Date.DAY), ComputerSystem.getInstance().getDate(Date.HOUR), ComputerSystem.getInstance().getDate(Date.MINUTE), ComputerSystem.getInstance().getDate(Date.SECONDE), ComputerSystem.getInstance().getDate(Date.SUNDAY), ComputerSystem.getInstance().getHoliDay(), DailyPaper.getInstance().getDailyImageHobby().get()));
    private Supplier<Object> promptSpawnDailyWallpaperHobbyPrompt = () -> ("请你结合图像内容和用户喜好，帮助用户优化用于生成图像的提示词，特别强调只需要回复要生成的图像的描述信息即可。\n*用户喜好：%s\n*图像内容：%s"); /// %s用户喜好 %s图像内容
