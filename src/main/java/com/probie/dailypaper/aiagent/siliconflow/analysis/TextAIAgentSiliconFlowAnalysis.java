...
import com.probie.dailypaper.aiagent.api.siliconflow.analysis.ITextAIAgentSiliconFlowAnalysis;

@Data
public class TextAIAgentSiliconFlowAnalysis extends AIAgentSiliconFlow implements ITextAIAgentSiliconFlowAnalysis {

    /**
     * 构造函数
     * */
    public TextAIAgentSiliconFlowAnalysis() {
        init();
    }
     ...
     * 维护一个懒加载的类单例对象
     * */
    private volatile static TextAIAgentSiliconFlowAnalysis INSTANCE;

    /**
     * 获取一个懒加载的类单例对象
     * */
    public synchronized static TextAIAgentSiliconFlowAnalysis getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new TextAIAgentSiliconFlowAnalysis();
        }
        return INSTANCE;
    }
    ...
    protected void init() {
        ...
        setAPIModel(DailyPaper.getInstance().getAPIModelTextSiliconFlowAnalysis());
        setAPIUrl(DailyPaper.getInstance().getAPIUrlTextSiliconFlowAnalysis());
    }
