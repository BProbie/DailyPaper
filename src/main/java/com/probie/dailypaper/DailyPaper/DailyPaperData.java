...
public class DailyPaperData implements IDailyPaperData {
    ...
    private SimpleObjectProperty<Object> promptIfImagePrompt = new SimpleObjectProperty<>("请你根据用户输入的上下文信息帮我推测判断出用户现在是否需要生成图像，是则回答单个字“是”，否则回答单个字“否”，无法推断或模糊不清则一律回答单个字“否”，以下是用户输入的上下文信息：");
    private SimpleObjectProperty<Object> promptSpawnImagePrompt = new SimpleObjectProperty<>("请你根据用户输入的上下文信息整理回复用户想生成图像的描述信息，稍微简短，尽量详备，并只需要回复要生成的图像的描述信息即可，以下是用户输入的上下文信息：");
    private SimpleObjectProperty<Object> promptSpawnImageResultPrompt = new SimpleObjectProperty<>("请你根据用户当前输入的用于生成图像提示词信息，返回该图像生成成功的回复，以下是用户输入的提示词信息：");
    ...
    private SimpleObjectProperty<Object> promptSpawnDailyWallpaperHobbyPrompt = new SimpleObjectProperty<>("请你结合图像内容和用户喜好，帮助用户优化用于生成图像的提示词，特别强调只需要回复要生成的图像的描述信息即可。\n*用户喜好：%s\n*图像内容：%s"); /// %s用户喜好 %s图像内容
