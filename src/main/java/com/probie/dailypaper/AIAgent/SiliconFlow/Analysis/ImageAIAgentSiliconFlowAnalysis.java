package com.probie.dailypaper.AIAgent.SiliconFlow.Analysis;
...
import com.probie.dailypaper.AIAgent.Interface.SiliconFlow.Analysis.IImageAIAgentSiliconFlowAnalysis;

@Data
public class ImageAIAgentSiliconFlowAnalysis extends AIAgentSiliconFlow implements IImageAIAgentSiliconFlowAnalysis {

    /**
     * 构造函数
     * */
    public ImageAIAgentSiliconFlowAnalysis() {
        init();
    }
     ...
     * 维护一个懒加载的类单例对象
     * */
    private volatile static ImageAIAgentSiliconFlowAnalysis INSTANCE;

    /**
     * 获取一个懒加载的类单例对象
     * */
    public synchronized static ImageAIAgentSiliconFlowAnalysis getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new ImageAIAgentSiliconFlowAnalysis();
        }
        return INSTANCE;
    }
    ...
    protected void init() {
        ...
        setAPIModel(DailyPaper.getInstance().getAPIModelImageSiliconFlowAnalysis());
        setAPIUrl(DailyPaper.getInstance().getAPIUrlImageSiliconFlowAnalysis());
    }

    @Override
    public String[] analysisImage(String fullFilePath, String prompt) {
        /// 设置请求参数
